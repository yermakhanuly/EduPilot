    ...
    <section className="assistant-workspace">
      <aside className="conversation-sidebar">
        <div className="conversation-sidebar-top">
          ...
          <button className="primary small" type="button" onClick={() => createConversation.mutate()} disabled={createConversation.isPending}>New chat</button>
        </div>
        ...
        <div className="conversation-list">
          ...
          {conversations.map((conversation) => (
            <article className={`conversation-row${conversation.id === displayedActiveId ? ' active' : ''}`} key={conversation.id}>
              <button type="button" className="conversation-select" onClick={() => setActiveId(conversation.id)}>
      ...
      <div className="conversation-main">
        {activeConversation ? <>
          <header className="conversation-header">
            ...
            <label className="conversation-course">Course context<select value={activeConversation.courseId ?? ''} onChange={(event) => assistantApi.updateConversation(activeConversation.id, { courseId: event.target.value || null }).then(() => { refreshConversations(); queryClient.invalidateQueries({ queryKey: ['assistant-messages', activeConversation.id] }) })}><option value="">All materials</option>{courses.map((course) => <option key={course.id} value={course.id}>{course.courseCode ? `${course.courseCode} · ${course.name}` : course.name}</option>)}</select></label>
          </header>
          ...
          {sendMessage.error || branchMessage.error || regenerateMessage.error ? <p className="error-text">{sendMessage.error?.message ?? branchMessage.error?.message ?? regenerateMessage.error?.message}</p> : null}
