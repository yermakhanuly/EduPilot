...
npm run test     # Vitest unit & route test suite
```
...
- **State**: Zustand for client-only state (`authStore`, `themeStore`, `strictStore`, `assistantStore`); React Query (@tanstack/react-query) for all server state
...
- `Course` and `KnowledgeDocument` — Canvas and user-created courses with uploaded/indexed learning materials
- `Conversation` and `ChatMessage` — Persistent multi-turn AI chat threads with branch lineage, sources, and action metadata
